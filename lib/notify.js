...
/* ---- daily brief (Phase 2 hero product) ---- */
/* Send the morning Brief everywhere. Takes pre-formatted messages so this
   module stays decoupled from lib/dailybrief:
   { tgFr, tgEn, waFr, waEn }. Channel gets FR; subscribers get their lang. */
async function sendDailyBrief(msgs){
  if (!isConfigured()) {
    console.log('\n[brief] No provider configured — Brief that WOULD be sent:\n' + stripHtml(msgs.tgFr) + '\n');
    return { sent: 0, logged: true };
  }
  const out = { channel: TG_CHANNEL() || null, channelResult: null, subscribersSent: 0 };
  if (TG_CHANNEL()) out.channelResult = await tgSendDetailed(TG_CHANNEL(), msgs.tgFr);
  const subs = store.load();
  for (const s of (subs.telegram || [])) {
    if (s.chat_id && await tgSend(s.chat_id, s.lang === 'en' ? msgs.tgEn : msgs.tgFr)) out.subscribersSent++;
  }
  for (const s of (subs.whatsapp || [])) {
    if (await waSend(s.contact, s.lang === 'en' ? msgs.waEn : msgs.waFr)) out.subscribersSent++;
  }
  out.sent = (out.channelResult && out.channelResult.ok ? 1 : 0) + out.subscribersSent;
  out.ok = out.sent > 0;
  return out;
...
module.exports = { markSeen, pushDigest, handleTelegramUpdate, isConfigured, digest, sendTest,
  newsletterMessage, pushNewsletter, sendDailyBrief };
