...
function clean(raw, extraStrip){
  ...
  // (e.g. "&lt;script&gt;") is turned into real tags and then removed —
  // otherwise it survives cleaning and becomes stored XSS when the frontend
  // renders it via innerHTML. Strip twice to catch tags revealed by decoding.
  let s = stripTags(decodeEntities(stripTags(raw))).replace(/\s+/g, ' ').trim();
