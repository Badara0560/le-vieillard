...
const FEEDS = [
  ...
  /* Francophone business wire — the West-Africa economy/FDI/markets angle. */
  { source: 'Financial Afrik',   cat: 'economy', lang: 'fr', url: 'https://www.financialafrik.com/feed/' },
...
function categorize(text, feed){
  ...
  /* Topical beats first — a story's subject wins over its source feed, so a
     sports or economy item from a Mali feed is no longer swallowed by "mali". */
  if (/football|\bfoot\b|\bcan\b|coupe d'afrique|coupe du monde|\bmatch\b|\bsport\b|joueur|aigles|basket|athl[ée]|olympique|\bligue\b|s[ée]lection|deschamps|mondial|\bfifa\b|\buefa\b|ballon d'or|yamal|mbapp[ée]|\bmessi\b|ronaldo|barcelone|bar[çc]a|real madrid|\bpsg\b|entra[îi]neur|stade/.test(s)) return 'sport';
  if (/[ée]conomie|[ée]conomique|franc cfa|\bcfa\b|\beco\b|banque|march[ée]|bourse|\bbrvm\b|\buemoa\b|\bafdb\b|\bbad\b|afcfta|zlecaf|\bor\b|coton|p[ée]trole|raffinerie|dangote|prix|inflation|investiss|\bide\b|\bfdi\b|\bfmi\b|croissance|budget|dette|commerce|douane|tarif|milliards? (?:de )?(?:dollars|fcfa|usd)/.test(s)) return 'economy';
  if (/num[ée]rique|\btech\b|startup|mobile money|internet|\bia\b|intelligence artificielle|application|fintech|digital|t[ée]l[ée]com/.test(s)) return 'tech';
  if (/musique|festival|\bfilm\b|cin[ée]ma|culture|patrimoine|\blivre\b|artiste|\bdanse\b/.test(s)) return 'culture';
  /* Then geography / politics. */
  if (feed.cat === 'mali' || /\bmali\b|bamako|malien|malienne|kayes|sikasso|s[ée]gou|mopti|tombouctou|\bgao\b|kidal|ansongo/.test(s)) return 'mali';
  if (/sahel|\bniger\b|burkina|\baes\b|alliance des [ée]tats|tchad|mauritanie|cedeao|ecowas/.test(s)) return 'sahel';
...
async function build(){
  ...
  all = all.slice(0, 90);   // room for cross-source clusters + fuller section bands
