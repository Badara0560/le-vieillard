...
function normEvent(e){
  ...
  const venue = c.venue || {};
  const alt = c.altGameNote || '';
  const gm = alt.match(/Group\s+([A-L])/i);
  const desc = st.description || '';
  // shootout / extra-time marker + "X advance on penalties" note
  let note = '';
  if (/penalt/i.test(desc)) note = 't.a.b.';
  else if (/extra time/i.test(desc)) note = 'a.p.';
  const adv = ((c.notes || []).find(n => /penalt|advance/i.test(n.headline || '')) || {}).headline || '';
  return {
    date: (e.date || '').slice(0, 10),
    iso: e.date || '',
    venue: (venue.address && venue.address.city) || venue.fullName || '',
    stage: (e.season && e.season.slug) || '',   // group-stage | round-of-32 | round-of-16 | quarterfinals | semifinals | final | third-place
    group: gm ? gm[1] : '',
    note, adv,
    winner: (home.winner ? 'home' : away.winner ? 'away' : ''),
